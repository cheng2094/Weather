...
object CommonUtils {
 ...
 fun getWeatherIcon(weather: List<Weather>?): String{
  return if (weather != null && weather.isNotEmpty()) weather[0].icon else "-"
 }

 fun Double.decimalFormat(digits: Int) = "%.${digits}fºC".format(this)

 fun Int.humidityFormat() = "Humidity: %d%%".format(this)
